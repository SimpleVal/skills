...
When `.kilo/agent/code-reviewer.md` exists, use the Code Reviewer Agent (code-reviewer) for both Review Axis tasks. Otherwise, use the normal review-capable general agent for both tasks and report that fallback in the aggregate output.
...
Present the two reports under `## Standards` and `## Spec` headings, verbatim or lightly cleaned. Do **not** merge or rerank findings — the two axes are deliberately separate (see _Why two axes_). Keep the Code Review Agent output from each Review Axis separate even when the same named agent performs both tasks.
